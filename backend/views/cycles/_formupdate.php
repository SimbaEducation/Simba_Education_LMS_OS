...
<!--                    <li>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="tab1">
                        ...
                        <?= $form->field($model, 'thumbnailupdate', ['template' => $template, 'labelOptions' => ['class' => $labelClass]])->fileInput(['maxlength' => true]) ?>

                        <div class="col-lg-3"></div>
                        <div class="col-lg-6">
                            <?php echo Html::img(Yii::getAlias('@web') . '/../uploads/cycles/' . $model['thumbnail'], ['width' => '120px']) ?>
                        </div>
                        ...
                        if (!empty($model->cycleGalleries[0])) {
                            foreach ($model->cycleGalleries AS $picture) {
                                ...
                                $initialPreviewConfig[] = ["caption" => $picture->picture, "width" => "120px", "url" => Url::to(['cycles/deletegallery', 'id' => $picture->id])];
                            }
                        }
                        echo '<div class="galleryUpdate">';
                        echo $form->field($model, 'gallery[]', ['template' => $template, 'labelOptions' => ['multiple' => true, 'class' => $labelClass]])->fileInput(['multiple' => true, 'maxlength' => true])->widget(FileInput::classname(), [
                            ...
                            'pluginOptions' => [
                                ...
                                'uploadUrl' => Url::to(['/cycles/imageuploadupdate', 'id' => $model->id]),
//                                'uploadExtraData' => [
//                                    'album_id' => 20,
//                                    'cat_id' => 'Nature'
//                                ],
                        ...
                        <div id="template-cycle-one">
                            <div class="table-scrollable">
                                <table class="table table-hover">
                                    ...
                                    <tr>
                                        <?php
                                        $cycleTemplate = CycleTemplates::findOne(['id'=>$model->cycle_template_id]);
                                            ...
                                            <td>
                                                <table class="table table-hover">
                                                    ...
                                                    <tr>
                                                        <th><?= $val->emphasis ?></th>
                                                    <tr>
                                                        <?php foreach (CycleSelectedActivity::findAll(['cycle_id'=>$model->id,'cycle_template_day_id'=>$val->id]) AS $cat) { ?>    
                                                        <tr>
                                                            <td style="color:white;font-weight:" bgcolor="<?= $cat->category->color_code ?>">
                                                                <?= Html::dropDownList('cycle_template_' . $cat->cycle_template_categories_day_id . '_' . $cat->category->id, $cat->activity_id, ArrayHelper::map(Activities::find()->where('category = ' . $cat->category_id . '')->all(), 'id', 'name'), ['prompt' => 'Select Activity', 'class' => 'form-control']) ?>
                                                                <label style="margin-top:5px;" class="label label-sm label-success pull-right"><?php // $cat->duration      ?> Mins</label>
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
