...
<div class="user-index">
    ...
    $gridColumns = [
        ...
        [
            ...
            'format' => 'raw',
            'value' => function($data) {
                return Html::img(Yii::getAlias('@web') . '/../uploads/school/' . $data['school_logo'], ['width' => '70px']);
            },
                ],
                'name',
                'address',
                ...
                [
                    'label' => 'Total Number of Student/Teacher',
                    'format' => 'raw',
                    'value' => function ($data) {
                        return $data->max_number_of_students . '/' . $data->max_number_of_teachers;
                    },
                ],
                [
                    'attribute' => 'school administrator',
                    'format' => 'raw',
                    'value' => function($data) {
                        return Html::a($data->username, ['user/view', 'id' => $data->user_id]);
                    },
                            'filter' => '<input class="form-control" value="" name="SchoolSearch[school_administrator]" type="text" placeholder="School Administrator">'
                        ],
                        [
                            ...
                            'buttons' => [
                                ...
                                if ($action === 'view') {
                                    $url = Url::to(['school/view', 'id' => $model->id]);
                                    return $url;
                                } else if ($action === 'update') {
                                    $url = Url::to(['school/update', 'id' => $model->id]);
                                    return $url;
                                } else if ($action === 'delete') {
                                    $url = Url::to(['school/delete', 'id' => $model->id]);
                                    return $url;
                                }
                            }
                                ],
...
//                    Create </span>
                                    ...
                                    ['content' =>
                                        Html::a('<i class="glyphicon glyphicon-plus"></i>', ['school/create'], ['title' => 'Add Activity Category', 'data-pjax' => '0', 'style' => 'padding:11px 14px 8px 14px !important;color:#fff !important', 'class' => 'btn btn-success']) . ' ' .
                                ...
                                'panel' => [
                                    ...
                                    'heading' => '<i style="margin-right:5px;" class="glyphicon glyphicon-tag pull-left"></i> Schools
    ',
                                    'footer' => false,
                                    'before' => '{pager}',
                                ],
                        ...
                        <script type="text/javascript">
                            $(document).ready(function () {
                                $("#school-pjax").on('pjax:complete', function () {
                            ...
                            function pageSize(status) {
                                window.location.href = '<?php echo Url::to(['/school/index']) ?>&per-page=' + status.value;
    }
