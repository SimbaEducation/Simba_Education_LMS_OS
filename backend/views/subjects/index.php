...
//                    'editableOptions' => ['header' => 'Name', 'size' => 'md']
        ...
        [
            'attribute' => 'age_short',
            'value' => function ($data) {
                return $data->age_short . '/' . $data->age_end;
            },
            'filter' => false,
        ],
        [
            'attribute' => 'standard',
            'value' => function ($data) {
                if (isset($data->standards->name)) {
                    return $data->standards->name;
                } else {
                    return 'Null';
                }
            },
        ...
        [
            ...
            'buttons' => [
                ...
                if ($action === 'view') {
                    $url = Url::to(['subjects/view', 'id' => $model->id]);
                    return $url;
                } else if ($action === 'update') {
                    $url = Url::to(['subjects/update', 'id' => $model->id]);
                    return $url;
                } else if ($action === 'delete') {
                    $url = Url::to(['subjects/delete', 'id' => $model->id]);
                    return $url;
                }
            }
                ],
...
//            </a> ',
                    ...
                    ['content' =>
                        Html::a('<i class="glyphicon glyphicon-plus"></i>', ['subjects/create'], ['title' => 'Add Activity Category', 'data-pjax' => '0', 'style' => 'padding:11px 14px 8px 14px !important;color:#fff !important', 'class' => 'btn btn-success']) . ' ' .
                ...
                'panel' => [
                    ...
                    'heading' => '<i style="margin-right:5px;" class="glyphicon glyphicon-tag pull-left"></i> Subjects
    ',
                    'footer' => false,
                    'before' => '{pager}',
                ],
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                $("#subjects-pjax").on('pjax:complete', function () {
            ...
            function pageSize(status) {
                window.location.href = '<?php echo Url::to(['/subjects/index']) ?>&per-page=' + status.value;
    }
